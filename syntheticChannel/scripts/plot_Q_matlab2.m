...
plot(timeHec, us_Q, 'k', ...
    'DisplayName','Upstream inflow', 'LineStyle', '-', 'Marker','o');

plot(timeHec, ds_Q, 'b', ...
    'DisplayName','Downstream (Dynamic)', 'LineStyle', '--', 'Marker','s', 'Color','k','MarkerIndices',1:6:length(timeHec));
...
plot(nwm_x(:,1)/3600, nwm_x(:,2), 'Color','m', ...
    'DisplayName','Downstream (CNS - Beg et al. 2023)', 'LineStyle', '-.', 'Marker','s','MarkerIndices',1:6:length(timeHec));

plot(nwm_t(:,1)/3600, nwm_t(:,2), 'Color',[1 0.5 0], ...
    'DisplayName','Downstream (CNT - Beg et al. 2023)', 'LineStyle', '-.', 'Marker','s','MarkerIndices',1:4:length(timeHec));
...
for i = 1:length(segmentDirs)
    ...
    plot(tvals/3600, downQ, 'k', ...
        'DisplayName','Downstream (Meshless)', 'LineStyle', '-', 'Marker','s','MarkerIndices',1:3:length(timeHec));
end
