...
hecuph = np.zeros(len(ds_Q))
hecdownh = np.zeros(len(ds_Q))
...
fig, axes = plt.subplots(2, 1, figsize=(7, 4), sharex='col')
axes[0].plot(timeHec, hecupQ, 'k', label='Upstream', linewidth=1, zorder=1)
axes[0].plot(timeHec, hecdownQ, label='Downstream (Dynamic)', linewidth=1.6, color='b', zorder=4)
axes[1].plot(timeHec, hecuph, color='b', label='Upstream (Dynamic)', linewidth=1, zorder=1)
axes[1].plot(timeHec, hecdownh, label='Downstream (Dynamic)', linewidth=1.6, color='b', zorder=4)
...
for seg in segments:
    ...
    # Plot results
    axes[0].plot(timeValues/3600, downstreamQ, 'k', label='Downstream (Meshless)', linewidth=1.6, zorder=5)
    axes[1].plot(timeValues / 3600, downstreamH, 'k', label='Downstream (Meshless)', linewidth=1.6, zorder=5)
    axes[1].plot(timeValues / 3600, upstreamH, 'k', label='Downstream (Meshless)', linewidth=0.8, zorder=5)
    axes[0].plot(nwm_x[:, 0]/3600, nwm_x[:, 1], label=r'Downstream (CNS - Beg $\mathit{et\,al.}$ 2023)', linewidth=1.6, color='r', zorder=3)
    axes[0].plot(nwm_t[:, 0] / 3600, nwm_t[:, 1], label=r'Downstream (CNT - Beg $\mathit{et\,al.}$ 2023)', linewidth=1.6, color='orange', zorder=2)
...
axes[0].set_ylim([19, 26])
axes[1].set_xlabel('Time (hr)', fontsize=12, fontname='Helvetica')
axes[0].set_ylabel(r'Discharge $(cms)$', fontsize=12, fontname='Helvetica')
axes[0].legend()
axes[1].legend()
# plt.savefig('ex1.pdf')
plt.show()
