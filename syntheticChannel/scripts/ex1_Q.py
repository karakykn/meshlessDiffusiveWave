...
axes.plot(timeHec, hecupQ, 'k', label='Upstream', linewidth=1, zorder=1)
axes.plot(timeHec, hecdownQ, label='Downstream (Dynamic)', linewidth=1.6, color='b', zorder=4)
...
for seg in segments:
    ...
    # Plot results
    axes.plot(timeValues/3600, downstreamQ, 'k', label='Downstream (Meshless)', linewidth=1.6, zorder=5)
    ...
    axes.plot(nwm_x[:, 0]/3600, nwm_x[:, 1], label=r'Downstream (CNS - Beg $\mathit{et\,al.}$ 2023)', linewidth=1.6, color='r', zorder=3)
    axes.plot(nwm_t[:, 0] / 3600, nwm_t[:, 1], label=r'Downstream (CNT - Beg $\mathit{et\,al.}$ 2023)', linewidth=1.6, color='orange', zorder=2)
...
axes.set_ylabel(r'Discharge $(cms)$', fontsize=12, fontname='Helvetica')
axes.legend()
# axes[1].legend()
# plt.savefig('ex1.pdf')
plt.show()
