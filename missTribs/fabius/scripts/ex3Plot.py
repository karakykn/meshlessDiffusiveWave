...
us2 = make_Meshless_df('../segment1/run', node_id=0, start_date=start_date)
ds = make_Meshless_df('../segment2/run', node_id=-1, start_date=start_date)

plt.plot(us['Date'], us['discharge-cms'], label='north')
plt.plot(us2['Date'], us2['discharge-cms'], label='south')
plt.plot(ds['Date'], ds['discharge-cms'], label='ds')
...
df_hourly_reset.to_csv("fabius.csv", index=False)
