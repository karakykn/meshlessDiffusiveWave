...
meshless_freeport = make_Meshless_df(f'../segment0/run', node_id=26, start_date=df_freeport['Date'].iloc[0])
fig, axes = plt.subplots(1, 1, figsize=(7, 4), sharex='col')
axes.plot(df_freeport['Date'], df_freeport['Q-cms'], label='USGS (Gage: 11447650)', marker='o', linestyle='None', markersize=1.6, color='b', zorder=3)
axes.plot(meshless_freeport['Date'], meshless_freeport['discharge-cms'], label='Meshless', linewidth=1.5, color='k', zorder=2)

compute_errors("Freeport", df_freeport, meshless_freeport)
