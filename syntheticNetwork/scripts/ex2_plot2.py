...
fig, axes = plt.subplots(n_segments, 2, figsize=(7, 3 * n_segments), sharex='col')
if n_segments == 1:
...
for idx, seg in enumerate(segments):
    ...
    if os.path.exists(us_file) and os.path.exists(ds_file):
        ...
        if idx == 2:
            axH.plot(timeHec, h_us, label='Upstream (Dynamic)', linewidth=.8, color='b', zorder=14)
        else:
            axH.plot(timeHec, h_us, label='Upstream (Dynamic)', linewidth=.8, color='b', zorder=14)
        axH.plot(timeHec, h_ds, label='Downstream (Dynamic)', linewidth=1.6, color='b', zorder=19)
    ...
    if timeValues:
        ...
        if len(upstreamH_Meshless) == len(timeValues):
            axH.plot(timeValues / 3600, upstreamH_Meshless, 'k', label='Upstream (Meshless)', linewidth=.8, zorder=15)

        # --- Plot Downstream ---
        axQ.plot(timeValues / 3600, downstreamQ, 'k', label='Downstream (Meshless)', linewidth=1.6, zorder = 20)
        axH.plot(timeValues / 3600, downstreamH, 'k', label='Downstream (Meshless)', linewidth=1.6, zorder = 20)
