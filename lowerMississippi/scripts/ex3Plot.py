...
observation_baton = pd.read_csv(f'{fPath}/../usgs_baton', sep='\t', skiprows=24)
CNX_baton = pd.read_csv(f'{fPath}/../beg_data/baton_disc_beg', sep='\t')
CNX_belle = pd.read_csv(f'{fPath}/../beg_data/belle_disc_beg', sep='\t')
observation_baton['Q-cms'] = observation_baton['14n'] / 35.31466621266132
...
# helper to build present DataFrame for a node
...
# present datasets
present_baton = make_present_df('../segment0/run', node_id=28, start_date=start_date)  # node 28 is baton
present_belle = make_present_df('../segment0/run', node_id=72, start_date=start_date)  # node 72 is belle chase
...
ax.plot(present_baton['Date'], present_baton['discharge-cms'], label='Present', linewidth=1.5, color='k', zorder=10)
...
ax.plot(present_belle['Date'], present_belle['discharge-cms'], label='Present', linewidth=1.5, color='k', zorder=10)
...
compute_errors("Baton Rouge", observation_baton, present_baton, CNX_baton)
compute_errors("Belle Chasse", observation_belle, present_belle, CNX_belle)
