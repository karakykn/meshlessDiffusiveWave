...
mi = 1:round(max(1, numel(x_baton)/20)):numel(x_baton);
...
mih = 1:round(max(1, numel(dateh)/30)):numel(dateh);
...
mi = 1:round(max(1, numel(date)/15)):numel(date);
% Plot
plot(ax1, date, q, ...
    'LineStyle','-', ...
    'Color','m', ...
...
mi = 1:round(max(1, numel(Meshless_baton{:,3})/25)):numel(Meshless_baton{:,3});
...
mi = 1:round(max(1, numel(Meshless_belle{:,3})/30)):numel(Meshless_belle{:,3});
