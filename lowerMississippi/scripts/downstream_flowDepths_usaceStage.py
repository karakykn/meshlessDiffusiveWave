...
thalweg = 14
df['t-sec'] = dum_df['t']
s = pd.to_numeric(df['Stage (Ft)'], errors='coerce')
df.loc[df['Stage (Ft)'] == 'M', 'Stage (Ft)'] = (s.shift(1) + s.shift(-1)) / 2
df['stage-m'] = df['Stage (Ft)'].astype(float) / 3.281 + thalweg
df.to_csv(f'inputs/downstreamFlowDepth', index=False, header=None, sep='\t', columns=['t-sec', 'stage-m'])
